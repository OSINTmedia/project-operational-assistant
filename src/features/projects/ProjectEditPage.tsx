...
import { saveProjectEdits } from '../../domain/projectRules'
...
export function ProjectEditPage() {
    ...
    return (
      <section className="grid gap-4">
        <div className="rounded-xl border border-slate-200 bg-panel p-3 shadow-panel sm:p-5">
          ...
          <h2 className="mt-2 text-2xl font-semibold text-slate-950">Edit Project</h2>
          <p className="mt-3 text-sm leading-6 text-slate-600">
            Loading project fields and role permissions.
          </p>
        </div>
      </section>
    )
      ...
      <section className="grid gap-4">
        <div className="rounded-xl border border-rose-200 bg-white p-3 shadow-panel sm:p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-rose-700">Project edit form unavailable</p>
  ...
  return <ProjectEditPageReady data={formShell.data} projectId={projectId ?? null} />
}

function ProjectEditPageReady({
  data,
  projectId,
}: {
  ...
  const [selectedStatusId, setSelectedStatusId] = useState<StatusId>(
    project?.status ?? data.statusOptions[0]?.id ?? 'new',
  )
  const [selectedOwnerId, setSelectedOwnerId] = useState(project?.ownerId ?? data.defaultOwnerId ?? '')
  const [selectedTeamId, setSelectedTeamId] = useState(project?.teamId ?? data.defaultTeamId ?? '')
  const [submitError, setSubmitError] = useState<string | null>(null)
  const [isSubmitting, setIsSubmitting] = useState(false)
  const projectDetailPath = projectId ? `/projects/${projectId}` : '/projects'
        ...
        <div className="rounded-xl border border-slate-200 bg-panel p-3 shadow-panel sm:p-5">
          <ContextBreadcrumbs
            items={[
              ...
              project ? { label: project.name, to: projectDetailPath } : { label: 'Project' },
              { label: 'Edit' },
            ]}
          />
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!currentUserId) {
      setSubmitError('A current demo user is required to save project changes.')
      return
    }

    if (!projectId) {
      setSubmitError('A valid project id is required.')
      return
    }
    ...
    try {
      const savedProject = await saveProjectEdits({
        projectId,
      ...
      setSubmitError(
        error instanceof Error ? error.message : 'Failed to save project changes in local demo data.',
      )
    ...
    <section className="grid gap-4">
        ...
        <div className="mt-4 flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
              Save returns to project detail. If all project issues are done, the project status is
              saved as Done.
            </p>
          </div>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-3 shadow-panel sm:p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-slate-950">Save project changes</p>
            <p className="mt-1 hidden text-sm leading-6 text-slate-600 sm:block">
              Project fields save through domain rules; issue-derived status remains authoritative.
            </p>
          </div>
        </div>
          ...
          <div className="sticky top-2 z-10 rounded-xl border border-slate-200 bg-white/95 p-2.5 shadow-panel backdrop-blur sm:top-3 sm:p-3">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="grid gap-1 text-sm text-slate-600">
                <p>
                  <span className="font-medium text-slate-950">After save:</span> return to
                  project detail
                </p>
                <p>
                  <span className="font-medium text-slate-950">Status rule:</span> all issues done
                  overrides to Done
                </p>
              </div>
              <div className="grid w-full gap-2 sm:grid-cols-2 lg:w-auto">
                <Link
                  to={projectDetailPath}
                  aria-label="Cancel project editing and return to project detail."
